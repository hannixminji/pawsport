...
class DashboardService:
        ...
        rows = (
            await self.db.execute(
                ...
                .where(
                    ...
                    MissingReport.report_status.in_(statuses)
                )
                .group_by(MissingReport.report_status)
            )
        ).all()
        result = {status.value: 0 for status in statuses}
    ...
    async def get_user_stats(self, user_id: int) -> dict[str, Any]:
        return {
            "pets": await self.get_user_pets_count(user_id),
            "missing_reports": await self.get_user_missing_reports_count(user_id),
            "sighting_reports": await self.get_user_sighting_reports_count(user_id),
        }
    ...
    async def get_all_stats(self) -> dict[str, Any]:
        mobile_users = await self.get_total_mobile_users()
        admin_users = await self.get_total_admin_users()
        pets = await self.get_total_pets()
        missing_reports_total = await self.get_total_missing_reports()
        missing_reports_by_status = await self.get_missing_reports_by_status()
        sighting_reports = await self.get_total_sighting_reports()
